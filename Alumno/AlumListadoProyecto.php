				...
				<table id="example" class="display table-striped table-hover"
					...
					<tbody>
						...
						while ($row = mysqli_fetch_array($result)) {
							...
							<tr>
								<td>
									<?php echo $row['SPNombreProyecto']; ?>
								</td>
								<td>
									<?php echo $row['SPObjetivo']; ?>
								</td>
								<td>
									<?php echo $row['SPDescripcion']; ?>
								</td>
								<td>
									<?php echo $row['SPEstudiantesRequeridos']; ?>
								</td>
								<td>
									<?php if ($candidato['candidato']) { ?>
										<form action="AlumSolicitudResidencia.php" method="Post">
											...
											<button type="submit" class="btn btn-outline-info"
												data-proyecto-id="<?php echo $row['SPEstatus']; ?>">
									...
									<?php } else { ?>
										<button type="button" class="btn btn-outline-info"
											onclick="window.alert('Tu perfil no cumple los requisitos para solicitar una residencia')">
											<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
